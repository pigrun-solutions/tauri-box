...
import { deleteNozzle } from '@/database/nozzles'
import { useNavigate } from '@tanstack/react-router'
import AlertModal from '@/components/ui/alert-modal'
import { useNozzleStore } from '@/zustand/nozzles-store'

const CellActions = ({ data }: { data: Nozzle }) => {
    ...
    const onDelete = async () => {
        try {
            ...
            const response = await deleteNozzle(data.id)
        ...
        <>
            ...
            <div className="flex gap-2 items-center justify-end h-full">
                <Button variant="ghost" size="icon" className="size-5" asChild>
                    <Edit3 className="text-primary cursor-pointer size-5" onClick={() => navigate({ to: `/dashboard/nozzles/${data.id}` })} />
                </Button>
